...
# User earns badge when he/she gets a likes a post.
@receiver(post_save, sender=Post)
def award_like_badge(sender, instance, created, **kwargs):
    if created:
        ...
        if post_count >= 1:
            badge = Badge.objects.get(name='Liker')
            user_badge, created = UserBadge.objects.get_or_create(user=user, badge=badge)
            if created:
                " send notification to user "
                user_badge.save()
                pass
        if post_count >= 2:
            badge = Badge.objects.get(name='Platonic Lover')
            user_badge, created = UserBadge.objects.get_or_create(user=user, badge=badge)
            if created:
                " send notification to user "
                user_badge.save()
        ...
        if post_count >= 3:
            badge = Badge.objects.get(name='Lover')
            user_badge, created = UserBadge.objects.get_or_create(user=user, badge=badge)
            if created:
                " send notification to user "
                user_badge.save()
...
# User earns badge when he/she has specific number of likes.
@receiver(post_save, sender=Post)
def award_like_badge(sender, instance, created, **kwargs):
    if created:
        user = instance.author
